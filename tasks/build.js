...
var serveDir = projectDir.cwd('./.tmp');
...
var copyDistTask = function () {
    return distDir.copyAsync(distDir.path(), destDir.path(), {
        overwrite: true,
		matching: ['!package.json']
    });
};

var copyElectronForDistTask = function () {
    return distDir.copyAsync(srcDir.path(), destDir.path(), {
        overwrite: true,
		matching: paths.copyFromDist
    });
};

gulp.task('copy', ['clean'], copyTask);

gulp.task('copyfordist', ['clean'], copyElectronForDistTask);

gulp.task('copy:dist', ['clean'], copyDistTask);
